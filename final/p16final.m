% EdX CS1156x Learning from Data Final Exam, Problems 16
% Author: David Eisner (deisner@gmail.com)

function [Ntbl] = p16final()
    ...
    for rep=1:N_reps
        ...
        % cols: E_in, E_out; one row for each K
        Evals = zeros(2,2);
        ...
        for K=Kvals
            ...
            Evals(ki,1) = sum(y_rbf ~= Xtrain(:,3))/numel(y_rbf);
            ...
            Evals(ki,2) = sum(y_rbf ~= Xtest(:,3))/numel(y_rbf);
        ...
        % We have a "good" run, with no empty clusters for either K
        
        deltaEin =  Evals(2,1) - Evals(1,1);
        deltaEout = Evals(2,2) - Evals(1,2);
